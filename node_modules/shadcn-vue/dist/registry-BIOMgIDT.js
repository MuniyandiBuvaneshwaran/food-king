import { a as BASE_COLORS, c as FALLBACK_STYLE, d as REGISTRY_URL, f as SHADCN_VUE_URL, i as BASES, l as FONTS, n as transformIcons, o as BUILTIN_REGISTRIES, p as STYLES, t as transformMenu, u as ICON_LIBRARIES } from "./transform-menu-CdBnB1yA.js";
import { C as searchResultsSchema, T as workspaceConfigSchema, _ as registryItemSchema, a as rawConfigSchema, b as registryResolvedItemsTreeSchema, h as registryItemFileSchema, l as registryConfigSchema, n as configSchema, o as registriesIndexSchema, r as iconsSchema, s as registryBaseColorSchema, u as registryIndexSchema, w as stylesSchema, x as registrySchema } from "./schema-_wQCad_c.js";
...
function applyRtlMapping(input) {
	return input.split(" ").flatMap((className) => {
		...
		for (const [variantPattern, physical, logical] of RTL_LOGICAL_SIDE_SLIDE_MAPPINGS) if (variant?.includes(variantPattern) && value.startsWith(physical)) {
			const mappedValue$1 = value.replace(physical, logical);
			return [modifier ? `${variant}:${mappedValue$1}/${modifier}` : `${variant}:${mappedValue$1}`];
		}
	...
	return {
		...
		transform({ scriptASTs, sfcAST, utils: { traverseScriptAST, traverseTemplateAST } }) {
			...
			for (const scriptAST of scriptASTs) traverseScriptAST(scriptAST, {
				visitLiteral(path$1) {
					if (path$1.parent.value.type === "ImportDeclaration") return this.traverse(path$1);
					if (typeof path$1.node.value === "string") {
						const originalValue = path$1.node.value;
						const newValue = applyClassMappings(originalValue, classMapping);
						if (newValue !== originalValue) {
							path$1.node.value = newValue;
							transformCount++;
						}
					}
					return this.traverse(path$1);
				},
				visitTemplateLiteral(path$1) {
					for (const quasi of path$1.node.quasis) if (quasi.value.raw) {
...
z.intersection(registryItemSchema, z.object({ _source: z.string().optional() }));
...
async function promptForPreset(options) {
	...
	const { selectedPreset } = await prompts({
		...
		choices: [...presets.map(([name, preset$1]) => ({
...
async function getProjectComponents(cwd) {
	...
	let entries;
	try {
		entries = await fsExtra.readdir(uiDir, { withFileTypes: true });
	} catch {
		return [];
	}
	const candidates = /* @__PURE__ */ new Set();
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			...
			let dirEntries;
			try {
				dirEntries = await fsExtra.readdir(componentDir, { withFileTypes: true });
			} catch {
				continue;
			}
...
//#endregion
export { mergeEnvContent as $, DEFAULT_PRESETS as A, handleError as B, getRegistryPresets as C, highlighter as Ct, getShadcnRegistryIndex as D, getRegistryVisualStyles as E, ensureRegistriesInConfig as F, isUrl$1 as G, validateRegistryConfigForItems as H, resolveRegistryTree as I, transformRtl as J, updateFiles as K, _createSourceFile as L, promptForPreset as M, resolveInitUrl as N, resolveRegistryItems as O, resolveRegistryBaseConfig as P, getNewEnvKeys as Q, _getQuoteChar as R, getRegistryPreset as S, RegistryUnauthorizedError as St, getRegistryVisualStyle as T, clearRegistryContext as U, buildUrlAndHeadersForRegistryItem as V, isUniversalRegistryItem as W, logger as X, spinner as Y, findExistingEnvFile as Z, getRegistryFonts as _, RegistryLocalFileError as _t, getProjectTailwindVersionFromConfig as a, DEFAULT_UTILS as at, getRegistryIcons as b, RegistryNotFoundError as bt, getItemTargetPath as c, findPackageRoot as ct, getRegistry as d, resolveConfigPaths as dt, composeStyleId as et, getRegistryBase as f, RegistriesIndexParseError as ft, getRegistryFont as g, RegistryInvalidNamespaceError as gt, getRegistryBases as h, RegistryForbiddenError as ht, getProjectInfo as i, DEFAULT_TAILWIND_CSS as it, promptForBase as j, resolveTree as k, getRegistriesConfig as l, getConfig as lt, getRegistryBaseColors as m, RegistryFetchError as mt, getProjectComponents as n, DEFAULT_COMPONENTS as nt, getPackageInfo as o, createConfig as ot, getRegistryBaseColor as p, RegistryError as pt, transform$2 as q, getProjectConfig as r, DEFAULT_TAILWIND_CONFIG as rt, fetchTree as s, findCommonRoot$1 as st, searchRegistries as t, configWithDefaults as tt, getRegistriesIndex as u, getWorkspaceConfig as ut, getRegistryIconLibraries as v, RegistryMissingEnvironmentVariablesError as vt, getRegistryStyles as w, getRegistryItems as x, RegistryParseError as xt, getRegistryIconLibrary as y, RegistryNotConfiguredError as yt, updateTailwindConfig as z };
//# sourceMappingURL=registry-BIOMgIDT.js.map